...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        arguments = rest.map(scheme_eval_helper)
...
def eval_all(expressions, env, _=None):
    ...
    if expressions is nil:
        ...
        sub_expression = expressions.second

        while sub_expression is not nil:

            if sub_expression.second is nil:
                ret = scheme_eval(sub_expression.first, env, True)
            else:
                ret = scheme_eval(sub_expression.first, env)

            sub_expression = sub_expression.second
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        evaluated = scheme_eval(expressions.second.first, env)
        ...
        lambda_expression = Pair(expressions.first.second, expressions.second)
        lambda_function = do_lambda_form(lambda_expression, env)
