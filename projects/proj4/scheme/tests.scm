...
high_order
; expect (lambda (x y) (define (helper a b) (+ (* a x) (* b y))) helper)

(high_order 1 2)
; expect (lambda (a b) (+ (* a x) (* b y)))

(define inside_high_order (high_order 1 2))
; expect inside_high_order
...
(define (square_distance x)
  (define (helper y)
    (+ (* x x) (* y y))
  )
  helper
)
; expect square_distance

(square_distance 3)
; expect (lambda (y) (+ (* x x) (* y y)))
...
(let ((x 2))
  (let ((x 4) (y x))  ; first x = 4; second x = 2
    (+ y (* x 2))
  )
)
; expect 10
...
(define mu_test (mu (x) (+ x (* y y))))
; expect mu_test

(define square_sum (lambda (x y) (mu_test (* x x))))
; expect square_sum
