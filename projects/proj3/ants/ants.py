...
class Place(object):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                assert self.ant.can_contain(insect) \
                        or insect.can_contain(self.ant), 'Two ants in {0}'.format(self)
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if not (isinstance(insect, QueenAnt) and insect.queen == True):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place is not None:
            ...
            if place is not hive and len(place.bees) != 0:

                if self.max_range is None and self.min_range is None:
                    return random_or_none(place.bees)
                elif self.min_range is None and count <= self.max_range:
                    return random_or_none(place.bees)
                elif self.max_range is None and count >= self.min_range:
                    return random_or_none(place.bees)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        if QueenAnt.first == True:
            self.queen = True
            QueenAnt.first = False
        else:
            self.queen = False

        super(QueenAnt, self).__init__()
            ...
            while place is not None:
                ...
                if ant is not None:
                    ...
                    if ant.container and ant.ant is not None and ant.ant not in QueenAnt.doubled_ants:
                        ant.ant.damage *= 2
                        QueenAnt.doubled_ants.append(ant.ant)
