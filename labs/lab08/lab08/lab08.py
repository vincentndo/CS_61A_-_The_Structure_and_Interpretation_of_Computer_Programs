    ...
    def helper(link):
        ...
        if link is Link.empty:
            ...
            if isinstance(link.first, Link):
                ret += helper(link.first)
            else:
                ret += str(link.first)

            return ret + mid + helper(link.rest) + back
...
def add_trees(t1, t2):
    ...
    branches = [ add_trees(b1, b2) for b1, b2 in zip(t1.branches, t2.branches) ]

    if len(t1.branches) > len(t2.branches):
        branches += [ add_trees(b, Tree(0)) for b in t1.branches[len(t2.branches):] ]
    elif len(t1.branches) < len(t2.branches):
        branches += [ add_trees(Tree(0), b) for b in t2.branches[len(t1.branches):] ]

    return Tree(t1.root + t2.root, branches)
