...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env.keys():
            ...
            raise NameError('No such a variable in environment!')
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        if self.operator.string in env.keys():
            list_of_Number = [ operand.eval(env) for operand in self.operands]
            return self.operator.eval(env).apply(list_of_Number)
        else:
            raise NameError('No such a variable in environment!')
